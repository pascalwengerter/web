  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      ...
      editorInstance.on('undoStackChanged', function (length) {
        ...
        const currentImageContentBuffer = Buffer.from(currentImageContent, 'base64')

        console.log(currentImageContentBuffer)

        emit('update:currentContent', currentImageContentBuffer)
      })
    })
